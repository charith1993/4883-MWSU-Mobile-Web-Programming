...
function geo_success(position) {
  ...
  if (position.coords.accuracy <= min_accuracy) {
    ...
    if (prev_lat != position.coords.latitude || prev_long != position.coords.longitude) {
      ...
      info_string =  position.coords.latitude + ", " + position.coords.longitude;
    }
  } else
  ...
  if (info_string) op.value = info_string;
}
...
function init_geo() {
	op= document.getElementById("current"); // Note the op is defined in global space and is therefore globally available
  ...
  if (op) {
    ...
    if (start_stop_btn) {
      start_stop_btn.onclick = function() {
        ...
        {
          start_stop_btn.innerHTML = "Determine Current Location";
